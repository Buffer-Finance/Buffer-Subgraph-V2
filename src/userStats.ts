...
import {
  _loadOrCreateABOptionStats,
  _loadOrCreateOptionStats,
} from "./initialize";
...
export function updateABTradeOpenStatsForUser(
  volume: BigInt,
  volumeUSD: BigInt,
  userAddress: string,
  optionContractAddress: string,
  ...
  const statsEntity = _loadOrCreateABOptionStats(
    optionContractAddress + "&" + userAddress,
    optionContractAddress,
    token,
    userAddress
...
export function updateABTradeClosingStatsForUser(
  userAddress: string,
  volume: BigInt,
  payout: BigInt,
  payout_usd: BigInt,
