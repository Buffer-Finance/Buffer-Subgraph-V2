...
function _updateDailyLeaderboard(
  ...
  let entity = _loadOrCreateLeaderboardEntity(
    _getDayId(timestamp),
    user,
    "daily"
  );
...
function _updateTotalLeaderboard(
  totalFee: BigInt,
  timestamp: BigInt,
  user: Bytes,
  isExercised: boolean,
  ...
  let entity = _loadOrCreateLeaderboardEntity("total", user, "total");
  entity.volume = entity.volume.plus(totalFee);
  entity.totalTrades += 1;
  entity.netPnL = isExercised
    ? entity.netPnL.plus(netPnL)
